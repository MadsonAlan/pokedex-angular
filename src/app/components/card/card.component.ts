...
const colours: any = {"aliceblue":"#f0f8ff","antiquewhite":"#faebd7","aqua":"#00ffff","aquamarine":"#7fffd4","azure":"#f0ffff",
...
export class CardComponent {
  ...
  constructor(private pokedexService: PokedexService, private pokeDataService: PokeDataService) { }
  ...
  ngOnInit() {
    this.pokedexService.getPokedex().subscribe(data => {
      data.results.map((result, index) => {
        this.pokeDataService.getPokedex(index + 1).subscribe(dataIntern => {
          this.pokemonList.push({
            ...
            name: result.name,
            url: result.url,
            color: this.hexToRgbA(colours[dataIntern.color.name.toLowerCase()])
          })
        });
      });
    });
